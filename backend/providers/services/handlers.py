...
class ProviderHandlers:
    ...
    def _classify_provider_message_fallback(self, message: str) -> str:
        ...
        insights_fields = [
            'property name:', 'gender preference:', 'total rooms:', 'available rooms:',
            'available slots', 'amenities', 'pricing', 'price/term:', 'price/month:',
            'price/week:', 'price/day:'
        ]
        insights_field_count = sum(1 for field in insights_fields if field in message_lower)
        if insights_field_count >= 3:
            return 'IS'
...
- Insights submitted successfully for {property_name}"""
            ...
            return {'success': False, 'message': 'Provider not found. Please contact support.'}
        except Exception as e:
            logger.error(f"Error handling provider response from {provider_phone}: {str(e)}", exc_info=True)
            return {'success': False, 'message': 'failed  processing provider response. Please try again.'}
        ...
        try:
            with transaction.atomic():
                ...
                if not content_sid:
                    ...
                    return {'success': False, 'message': 'Template configuration missing.'}
                ...
                if not success:
                    ...
                    return {'success': False, 'message': 'failed to send confirmation to student. Contact support team.'}
    ...
    def _process_provider_rejection(self, booking: Booking, message: str) -> Dict:
        """Process provider rejection without payment or token handling."""
        try:
            with transaction.atomic():
                ...
                content_variables = {
                    ...
                    "3": booking.property.provider.name,
                    "4": student_name,
                    "5": booking.cell_number
                }

                success = self.whatsapp_service.send_template_message(booking.cell_number, content_sid, content_variables)
                if not success:
                    logger.error("Failed to send rejection message to student")
                    return {'success': False, 'message': 'failed sending rejection to student.'}
                ...
                logger.info(f"Booking {booking.booking_number} rejected by provider")
                return {
                    'success': True,
            ...
            return {'success': False, 'message': 'failed processing rejection. Please try again.'}
    ...
    def _process_provider_info_request(self, booking: Booking, message: str) -> Dict:
        ...
        try:
            ...
            if not questions:
                return {'success': False, 'message': 'Could not identify specific questions in your message.'}
            ...
            if not success:
                ...
                return {'success': False, 'message': 'failed sending info request to student.'}
            ...
            return {'success': False, 'message': 'failed processing information request. Please try again.'}
    ...
    def _generate_property_list_message(self, provider: AccommodationProvider) -> Dict:
        ...
        try:
            ...
            if not properties.exists():
                return {'success': True, 'message': 'You have no properties listed yet. Please submit property insights to add properties.'}
            ...
            return {'success': False, 'message': 'Failed to retrieve property listings. Please try again later.'}
    ...
    def send_booking_message_to_provider(self, booking: Booking) -> Dict:
        ...
        try:
            ...
            if success:
                ...
                return {'success': False, 'message': 'failed sending message to provider.'}

        except Exception as e:
            logger.error(f"Error sending booking message: {str(e)}")
            return {'success': False, 'message': 'failed sending booking message.'}
    ...
    def _handle_provider_cancel_conversation(self, provider_phone: str) -> Dict:
        ...
        try:
            ...
            if conversation:
                ...
                logger.warning(f"No active conversation state found for student {booking.cell_number}")
                return {'success': True, 'message': 'No active conversation found to cancel.'}

        except Exception as e:
            logger.error(f"Error handling provider cancel conversation: {str(e)}")
            return {'success': False, 'message': 'Failed to cancel conversation. Please try again.'}
    ...
    def _generate_jeff_about_message_for_provider(self) -> Dict:
        ...
        try:
            ...
            return {'success': True, 'message': provider_message}
        except FileNotFoundError:
            logger.error("jeff_about.md file not found for provider.")
            return {'success': False, 'message': "Sorry, I couldn't retrieve the information at the moment. Contact support team"}
        except Exception as e:
            logger.error(f"Error reading jeff_about.md file for provider: {e}")
            return {'success': False, 'message': "Sorry, an error occurred while retrieving the information."}
