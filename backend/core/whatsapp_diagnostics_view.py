...
def _authorized(request):
    if getattr(getattr(request, "user", None), "is_authenticated", False) and getattr(request.user, "is_staff", False):
        return True
    try:
...
def _serialize(event):
    return {"time": event.created_at.isoformat(), "event_id": event.event_id, "correlation_id": event.correlation_id,
            "direction": event.direction, "event_type": event.event_type, "stage": event.stage, "status": event.status,
            "phone_last4": event.phone_last4, "external_id": event.external_id, "duration_ms": event.duration_ms,
            "error": event.error_message, "metadata": event.metadata}
...
def _snapshot(request):
    ...
    if correlation_id: qs = qs.filter(correlation_id=correlation_id)
    if phone_last4: qs = qs.filter(phone_last4=phone_last4[-4:])
    events = list(qs.order_by("-created_at")[:200])
    grouped = {}
    for event in reversed(events): grouped.setdefault(event.correlation_id, []).append(event)
    now = timezone.now()
    flows = []
    for correlation, flow_events in grouped.items():
        ...
        if failed: health, stuck_at = "failed", failed.stage
        elif latest.status == "started" and age_seconds >= 10: health, stuck_at = "stuck", latest.stage
        else: health, stuck_at = "ok", None
        flows.append({"correlation_id": correlation, "health": health, "stuck_at": stuck_at,
                      "age_seconds": round(age_seconds, 1), "last_event": _serialize(latest),
                      "events": [_serialize(e) for e in flow_events]})
    return {"status": "ok", "generated_at": now.isoformat(), "window": "24h",
...
def whatsapp_diagnostics(request):
    ...
    if request.GET.get("format") == "json": return JsonResponse(data)
    rows = []
    for flow in data["flows"]:
        badge = {"ok": "OK", "failed": "FAILED", "stuck": "STUCK"}[flow["health"]]
        rows.append(f"<tr><td><code>{flow['correlation_id']}</code></td><td>{badge}</td>"
...
<title>Jeff WhatsApp Diagnostics</title><style>body{{font:14px system-ui;margin:32px;background:#0b1020;color:#e8edf7}}
table{{width:100%;border-collapse:collapse}}th,td{{padding:10px;border-bottom:1px solid #29324a;text-align:left}}code{{color:#a9d5ff}}
