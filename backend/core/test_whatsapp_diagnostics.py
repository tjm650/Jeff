...
class WhatsAppDiagnosticsTests(TestCase):
    def test_event_model_persists_pipeline_event(self):
        event = WhatsAppDiagnosticEvent.objects.create(
            event_id='test-event-1', correlation_id='wamid.test.1', direction='inbound',
            event_type='message_received', stage='message_extracted', status='ok',
            phone_last4='4567', external_id='wamid.test.1', metadata={'message_type': 'text'},
        )
    ...
    def test_status_event_can_be_correlated_to_outbound_message(self):
        WhatsAppDiagnosticEvent.objects.create(
            event_id='outbound-1', correlation_id='wamid.inbound.1', direction='outbound',
            event_type='message_send', stage='meta_api_accepted', status='ok',
            phone_last4='4567', external_id='wamid.outbound.1',
        )
        payload = {
        ...
        with patch('whatsapp.whatsapp_handler._verify_meta_signature', return_value=True):
            ...
            request = RequestFactory().post('/webhook/whatsapp/', data=json.dumps(payload), content_type='application/json')
            response = diagnostic_whatsapp_webhook(request)
